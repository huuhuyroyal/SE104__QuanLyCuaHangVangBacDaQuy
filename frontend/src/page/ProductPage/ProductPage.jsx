...
const ProductPage = () => {
  // --- INIT STATE ---
  const [data, setData] = useState([]); // Initialize empty array to avoid errors
  ...
  const handleDeleteProducts = () => {
    ...
    if (activeItems.length > 0 && inactiveItems.length === 0) {
      ...
      // If only closed items selected -> ask for permanent deletion
      title = "Permanent delete warning";
      content = `Are you sure you want to PERMANENTLY DELETE ${inactiveItems.length} products? `;
  ...
  const handleEditProduct = (record) => {
    ...
    // Handle displaying existing image
    if (record.image) {
      setFileList([
        {
          uid: "-1",
