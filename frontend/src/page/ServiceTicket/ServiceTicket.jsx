...
import { Table, Button, Modal, Form, Input, Select, InputNumber, Row, Col, message, Tag, Space, Descriptions } from "antd";
...
const ServiceTicket = () => {
    ...
    const handleUpdateStatus = async (status) => {
        ...
        const res = await ticketService.updateServiceTicketStatus(selectedTicket.ticket.SoPhieuDV, status);
        if(res.errCode === 0) {
    ...
    const columns = [
        ...
        { title: 'Ngày lập', dataIndex: 'NgayLap', render: (val) => new Date(val).toLocaleDateString('vi-VN') },
        { title: 'Tổng tiền', dataIndex: 'TongTien', render: (val) => formatCurrency(val) },
        { title: 'Trả trước', dataIndex: 'TongTienTraTruoc', render: (val) => formatCurrency(val) },
        { title: 'Còn lại', dataIndex: 'TongTienConLai', render: (val) => <span style={{color: 'red', fontWeight: 'bold'}}>{formatCurrency(val)}</span> },
        { title: 'Tình trạng', dataIndex: 'TinhTrang', render: (val) => <Tag color={val === 'Hoàn thành' ? 'green' : 'orange'}>{val}</Tag> },
        {
            title: 'Thao tác',
            render: (_, record) => (
                <Button icon={<EyeOutlined />} onClick={() => handleViewDetail(record)} size="small">Xem</Button>
            )
        }
    ];
        ...
        <div className="main">
             <header className="Unit-header">
                <Input.Search 
                    ...
                    style={{ width: 300 }}
                    onChange={(e) => { setSearchText(e.target.value); fetchData(e.target.value); }}
                />
                <div className="add-section">
                    <Button danger icon={<DeleteOutlined />} disabled={selectedRowKeys.length === 0} onClick={handleDelete}>Xóa</Button>
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => { setIsCreateOpen(true); form.resetFields(); setItems([]); }}>Tạo phiếu dịch vụ</Button>
                </div>
            </header>
                    ...
                    <Row gutter={16}>
                        ...
                        <Col span={8}>
                            <Form.Item name="MaKH" label="Khách hàng" rules={[{required: true, message: "Vui lòng chọn khách hàng"}]}>
                                ...
                                >
                                    {customers.map(c => <Select.Option key={c.MaKH} value={c.MaKH}>{c.TenKH} - {c.SoDienThoai}</Select.Option>)}
                                </Select>
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                            <Form.Item name="NgayLap" label="Ngày lập" initialValue={new Date().toISOString().split('T')[0]}>
                                <Input type="date" />
                            </Form.Item>
                        </Col>
                    </Row>
                    ...
                    <Row gutter={16}>
                        <Col span={8}>
                            <Form.Item name="TongTien" label="Tổng tiền">
                                <InputNumber style={{width: '100%'}} readOnly formatter={val => `${val}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} />
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                        ...
                        <Col span={8}>
                            <Form.Item name="TongTienConLai" label="Còn lại">
                                <InputNumber style={{width: '100%', color: 'red', fontWeight: 'bold'}} readOnly formatter={val => `${val}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} />
                            </Form.Item>
                        </Col>
                    </Row>
                </Form>
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key="complete" type="primary" onClick={() => handleUpdateStatus("Hoàn thành")}>Hoàn thành</Button>,
                ...
                {selectedTicket && (
                    <div>
                        <Descriptions bordered size="small" column={2}>
                            <Descriptions.Item label="Số phiếu">{selectedTicket.ticket.SoPhieuDV}</Descriptions.Item>
                            <Descriptions.Item label="Ngày lập">{new Date(selectedTicket.ticket.NgayLap).toLocaleDateString()}</Descriptions.Item>
                            <Descriptions.Item label="Khách hàng">{selectedTicket.ticket.TenKH}</Descriptions.Item>
                            <Descriptions.Item label="SĐT">{selectedTicket.ticket.SoDienThoai}</Descriptions.Item>
                            <Descriptions.Item label="Tình trạng"><Tag>{selectedTicket.ticket.TinhTrang}</Tag></Descriptions.Item>
                        </Descriptions>
                        <br/>
                        <Table 
                        ...
                        <div style={{textAlign: 'right', marginTop: 10}}>
                             <h3>Tổng còn lại phải thanh toán: {formatCurrency(selectedTicket.ticket.TongTienConLai)}</h3>
                        </div>
                    </div>
                )}
            </Modal>
